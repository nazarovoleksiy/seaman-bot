...
// разовая миграция локальной БД -> /data
try {
...
// usage (счётчик ведём как раньше, по дням) + пожизненный лимит
...
`);
export function incUsage(tgId){ qUpsertInc.run(String(tgId), todayUTC()); }

// пожизненный лимит (сумма всех дней)
export const FREE_TOTAL_LIMIT = Number(process.env.FREE_TOTAL_LIMIT || 50);
const qGetTotal = db.prepare(`SELECT COALESCE(SUM(count),0) AS total FROM usage_log WHERE tg_id=?`);
export function totalUsage(tgId){ return qGetTotal.get(String(tgId)).total || 0; }
export function canUseLifetime(tgId){
    const used = totalUsage(tgId);
    return { ok: used < FREE_TOTAL_LIMIT, used, limit: FREE_TOTAL_LIMIT, left: Math.max(FREE_TOTAL_LIMIT - used, 0) };
}

// feedback
const qSaveFeedback = db.prepare(`INSERT INTO feedback (tg_id, username, text) VALUES (?,?,?)`);
