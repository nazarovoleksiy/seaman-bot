...
import { reasonWithConsensus } from '../../utils/reason.js'; // <-- ВАЖНО: только импорт
    ...
    bot.on('photo', async (ctx) => {
        ...
        try {
            ...
            if (!answerObj) {
                ...
                // 3) Решение (консенсус)
                const r = await withConcurrency(() => reasonWithConsensus(ocr, lang, 3));
                if (!r || !r.answer_letter) {
                    const msg = lang === 'ru'
